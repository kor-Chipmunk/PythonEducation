...
class MessageUtil:
    # send() 메소드는 msg 매개변수가 담고있는 모든 바이트를 내보낼 때까지 반복해서 socket.send() 메소드 호출
    @staticmethod
    def send(sock, msg):
        sent = 0
        buffer = msg.GetBytes()
    ...
    def receive(sock):
        ...
        # 헤더 읽기
        # 첫 반복문에서는 스트림으로부터 메시지 헤더의 경계를 끊어냅니다.
        while sizeToRead > 0:
        ...
        # 첫 반복문에서 얻은 헤더에섯 본문의 길이를 뽑아내어 그 길이만큼 다시 스트림으로부터 본문을 읽습니다.
        while sizeToRead > 0:
            buffer = sock.recv(sizeToRead)
            if not buffer:
                return None
